    ...
    <div class="container mx-auto py-12">
        <article class="bg-white shadow-md rounded-lg p-8 w-96 mx-auto">
            ...
            <form action="{{ route('property.store') }}" method="post" class="flex flex-col space-y-4" enctype="multipart/form-data">
                ...
                <input type="text" name="street" id="street" value="{{ old('street') }}" autofocus class="border-gray-300 rounded-md shadow-sm input input-enabled">
                @error('street')
                <div class="text-red-700 mt-2">{{ $message }}</div>
                @enderror

                <label for="name" class="text-black">Stad:</label>
                <input type="text" name="city" id="city" value="{{ old('city') }}" autofocus class="border-gray-300 rounded-md shadow-sm input input-enabled">
                @error('city')
                <div class="text-red-700 mt-2">{{ $message }}</div>
                @enderror

                <label for="name" class="text-black">Huisnummer:</label>
                <input type="text" name="house_number" id="house_number" value="{{ old('house_number') }}" autofocus class="border-gray-300 rounded-md shadow-sm input">
                @error('house_number')
                <div class="text-red-700 mt-2">{{ $message }}</div>
                @enderror
